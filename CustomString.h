...
public:
    ...
    int find_last_not_of(char ch) const 
    {
        ...
        return -1; // Return -1 if all characters match the given character
    }
    ...
    String substr(int start, int len = -1) const 
    {
        ...
        {
            return String(""); // Return an empty string for invalid start
        }

        int end = (len == -1 || start + len > size) ? size : start + len;
    ...
    void erase(int start, int count) {
        if (start < 0 || start >= size || count <= 0) return;

        int newLength = size - count;
        if (newLength < start) newLength = start;

        char* newStr = new char[newLength + 1];
        int index = 0;

        // Copy characters before the start position
        for (int i = 0; i < start; ++i) {
            newStr[index++] = arr[i];
        }

        // Skip the `count` characters to erase
    ...
    void erase(int start) {
        if (start < 0 || start >= size) return; // Validate start position

        char* newStr = new char[start + 1]; // Allocate space for the new string
        for (int i = 0; i < start; ++i) {
            newStr[i] = arr[i];
        }
        newStr[start] = '\0'; // Null-terminate the string
